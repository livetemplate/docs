...
func (c *AuthController) Login(state AuthState, ctx *livetemplate.Context) (AuthState, error) {
	...
	// Reload self (POST-Redirect-GET). The empty target is emitted as a
	// relative reference the browser resolves against the un-stripped
	// request URL, so this works at any mount without the recipe knowing
	// its own prefix. The new GET re-renders the template; IsLoggedIn=true
	// is now persisted, so the dashboard branch renders and the WebSocket
	// then connects.
...
func (c *AuthController) Logout(state AuthState, ctx *livetemplate.Context) (AuthState, error) {
	...
	// Reload self: the login form renders again at the same mount.
	return state, ctx.Redirect("", http.StatusSeeOther)
}
