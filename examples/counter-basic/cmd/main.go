// Command counter-basic starts the no-pubsub counter recipe as a standalone
// HTTP server. Production deployments embed the recipe via the docs-site
// cmd/site aggregator; this entry point exists so developers can run
// `go run ./examples/counter-basic/cmd` to iterate on the recipe in
// isolation, and so the cross-repo test harness can drive a real browser
...
func main() {
	...
	log.Printf("counter-basic listening on :%s", port)
	if err := http.ListenAndServe(":"+port, counterbasic.Handler(opts...)); err != nil {
		log.Fatal(err)
	}
}
