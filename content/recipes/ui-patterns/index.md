...
The recipes binary (`cmd/site`) compiles the UI pattern catalog metadata directly from `data.go`, so any pattern added to `docs/content/recipes/patterns/_app/` shows up in this catalog on the next deploy — no separate sync needed.
