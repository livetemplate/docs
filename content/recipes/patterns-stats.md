...
The same `patterns` data source that powers the [catalog page](/recipes/ui-patterns/) drives this stats overview — a different view of identical data. This is the "one source, many views" pattern: a docs site can present REST data from many angles without re-fetching, re-modelling, or duplicating the source.
    ...
    <table>
        ...
        {{range .Data}}
            <tr data-test="cat-row" data-category="{{.Slug}}">
                <td>
                    <a href="/recipes/ui-patterns/#cat-{{.Slug}}"><strong>{{.Name}}</strong></a>
                </td>
                <td>{{len .Patterns}}</td>
                <td>
                    {{if .Patterns}}
...
This page binds to the **same `patterns` source as `/recipes/ui-patterns/`** — declared once in `tinkerdown.yaml` and reused by any number of pages. The catalog renders a per-category listing; this page renders a tabular summary with sample patterns per category.
